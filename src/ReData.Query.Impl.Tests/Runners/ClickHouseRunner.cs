using ClickHouse.Client.ADO;
using ClickHouse.Client.Utility;
using ReData.Query.Impl.QueryBuilders;
using Testcontainers.ClickHouse;

namespace ReData.Query.Impl.Tests.Runners;

public class ClickHouseRunner: ISqlRunner {
    private ClickHouseContainer Container { get; set; } = null!;
    private ClickHouseConnection Connection { get; set; } = null!;

    public IQueryBuilder QueryBuilder { get; } = new QueryBuilderFactory().Create(DatabaseType.ClickHouse);
    
    public bool Started { get; private set; }

    public async Task InitializeAsync()
    {
        Started = true;
        Console.WriteLine("Создаём ClickHouse");
        Container = new ClickHouseBuilder().WithName("ClickHouse-Tests").Build();
        await Container.StartAsync();
        Connection = new ClickHouseConnection(Container.GetConnectionString());
        await Connection.OpenAsync();
        await Connection.ExecuteStatementAsync(TestTableCreate); 
        await Connection.ExecuteStatementAsync(TestTableFill); 
    }

    private string TestTableCreate = """
                                  CREATE TABLE "TestTable" (
                                      "id" Int32,
                                      "Name" String,
                                      "MaxScore" Decimal(10,2)
                                  ) Engine = MergeTree() ORDER BY id;
                                  """;

    private string TestTableFill =
                                  """
                                  INSERT INTO "TestTable" ("id", "Name", "MaxScore") VALUES 
                                  (1,'George', 22.0),
                                  (2,'Tom', 17.0),
                                  (3,'Tim', 18.0),
                                  (4,'Harry',21.0),
                                  (5,'Ben',15.0),
                                  (6,'Bob',18.0),
                                  (7,'Phoebe',21.0),
                                  (8,'Max', 14.0),
                                  (9,'Lary', 17.0),
                                  (10,'Zach', 15.0)
                                  """;

    public async Task DisposeAsync()
    {
        await Connection.DisposeAsync();
        await Container.StopAsync();
        await Container.DisposeAsync();
    }

    public async Task<object?> Scalar(string sql)
    {
        var result = await Connection.ExecuteScalarAsync(sql);
        return result;
    }


    public async Task<List<Player>> QueryAsync(string sql)
    {
        var reader = await Connection.ExecuteReaderAsync(sql);
        List<Player> result = new List<Player>();
        while (await reader.ReadAsync())
        {
            int id;
            try
            {
                id = reader.GetInt32(0);
            }
            catch
            {
                id = reader.GetByte(0);
            }
            Decimal maxScore;
            try
            {
                maxScore = reader.GetDecimal(2);
            }
            catch
            {
                maxScore = (decimal)reader.GetDouble(2);
            }
            result.Add(new Player()
            {
                id = id,
                Name = reader.GetString(1),
                MaxScore = maxScore
            });
        }
        return result;
    }
}

