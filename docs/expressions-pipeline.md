...
- Для неагрегатных константных выражений runtime не нужен (выражение остается inline).
- Для агрегатных выражений используется `IConstantRuntime`, при этом вычисление делается в том же query-scope, где были разрешены поля аргумента.
